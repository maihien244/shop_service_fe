import { clsx } from 'clsx'
import { type ColumnDef, type SortingState } from '@tanstack/react-table'
import { type WarehouseDto } from '../dto/warehouse.dto'
import { Eye, Pencil, Trash2 } from 'lucide-react'
import * as Button from '#/components/ui/button'

export const getSortQuery = (sorting: SortingState): string => {
    if (!sorting || sorting.length === 0) return 'create_at,desc'
    const s = sorting[0]
    const fieldMap: Record<string, string> = {
        createAt: 'create_at',
        updateAt: 'update_at',
        name: 'name',
        address: 'address',
        id: 'id'
    }
    const field = fieldMap[s.id] || s.id
    return `${field},${s.desc ? 'desc' : 'asc'}`
}

export type WarehouseColumnDefProps = {
    onEdit: (warehouse: WarehouseDto) => void
    onDelete: (warehouse: WarehouseDto) => void
    onPreview?: (warehouse: WarehouseDto) => void
}

export const getWarehouseColumnDef = (props: WarehouseColumnDefProps): ColumnDef<WarehouseDto>[] => [
    {
        accessorKey: 'id',
        header: 'ID',
        cell: (info) => <span className="font-medium text-text-sub-600">{info.getValue() as number}</span>,
        enableSorting: true,
    },
    {
        accessorKey: 'name',
        header: 'Tên kho',
        cell: (info) => (
            <div className="max-w-md truncate font-semibold text-text-strong-950 dark:text-static-white">
                {info.getValue() as string}
            </div>
        ),
        enableSorting: true,
    },
    {
        accessorKey: 'address',
        header: 'Địa chỉ',
        cell: (info) => (
            <div className="max-w-xs truncate text-text-sub-600 dark:text-text-soft-400">
                {info.getValue() as string}
            </div>
        ),
        enableSorting: false,
    },
    {
        accessorKey: 'isActive',
        header: 'Trạng thái',
        cell: (info) => {
            const isActive = info.getValue() as number
            return (
                <span
                    className={clsx(
                        "inline-flex items-center rounded-full px-2.5 py-0.5 text-label-xs font-semibold uppercase tracking-wider",
                        isActive === 1 && "bg-success-lighter text-success-base dark:bg-success-dark dark:text-static-white",
                        isActive === 0 && "bg-neutral-lighter text-neutral-sub dark:bg-neutral-dark dark:text-static-white"
                    )}
                >
                    {isActive === 1 ? 'Hoạt động' : 'Không hoạt động'}
                </span>
            )
        },
        enableSorting: false,
    },
    {
        accessorKey: 'createAt',
        header: 'Ngày tạo',
        cell: (info) => {
            const date = info.getValue() as string
            return date ? new Date(date).toLocaleDateString() : 'N/A'
        },
        enableSorting: true,
    },
    {
        id: 'actions',
        header: 'Thao tác',
        cell: (info) => {
            return (
                <div className="flex items-center gap-3">
                    <Button.Root variant='primary' size='xsmall' mode='ghost' onClick={() => props.onEdit(info.row.original)}>
                        <Button.Icon as={Pencil} />
                    </Button.Root>

                    {props.onPreview && (
                        <Button.Root variant='neutral' size='xsmall' mode='ghost' onClick={() => props.onPreview!(info.row.original)}>
                            <Button.Icon as={Eye} />
                        </Button.Root>
                    )}

                    <Button.Root variant='error' size='xsmall' mode='ghost' onClick={() => props.onDelete(info.row.original)}>
                        <Button.Icon as={Trash2} />
                    </Button.Root>
                </div>
            )
        },
        enableSorting: false,
    },
]
