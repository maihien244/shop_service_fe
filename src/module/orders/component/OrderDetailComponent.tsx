...
export function OrderDetailComponent({ orderId }: OrderDetailProps) {
        ...
        <div className="min-h-screen bg-[#f4f6f8] text-[#1f2937] dark:bg-bg-white-0 dark:text-static-white py-8 px-4 transition-colors duration-300">
            <div className="max-w-4xl mx-auto space-y-5">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-gray-500">
                    <Link to={"/users/orders" as any} className="hover:text-[#d70018] flex items-center gap-1 transition-all">
                        <ChevronLeft size={16} /> Lịch sử mua hàng
                    </Link>
                    <span className="text-gray-300">/</span>
                    <span className="text-[#d70018] font-black">Chi tiết đơn hàng</span>
                </div>

                {/* Warning Alert Box */}
                <div className="bg-[#fef8e8] dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-xl p-4 flex gap-3 text-xs text-amber-800 dark:text-amber-400 font-semibold leading-relaxed shadow-sm">
                    <AlertTriangle size={18} className="text-amber-600 dark:text-amber-500 shrink-0 mt-0.5" />
                    <div>
                        <span className="font-extrabold uppercase block mb-0.5">Cảnh báo bảo mật:</span>
                        Quý khách chỉ thanh toán sau khi đã nhận và kiểm tra hàng đầy đủ. Tuyệt đối không nhấp vào các đường link lạ tự xưng là từ cửa hàng, tránh trường hợp kẻ gian giả danh để thực hiện hành vi gian lận.
                    </div>
                </div>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-5 w-full sm:w-auto mt-2 sm:mt-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-neutral-50">
                                    <div className="text-left sm:text-right">
                                        <span className="text-xs font-black text-gray-800 dark:text-white block">
                                            {formatPrice(order.total / totalQuantity)}
                                        </span>
                                    ...
                                    <Link
                                        to="/public/laptops"
                                        className="border border-[#d70018] text-[#d70018] hover:bg-red-50/5 dark:hover:bg-red-955/10 font-bold text-[11px] px-3.5 py-1.5 rounded-lg transition-all text-center cursor-pointer shrink-0"
                                    >
                                        Mua lại
                                    </Link>
                                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start">
                    ...
                    {/* Left Column: Customer details & support */}
                    <div className="w-full md:col-span-7 space-y-4">
                        ...
                        <div className="bg-white dark:bg-bg-weak-50 border border-neutral-200 dark:border-stroke-sub-300 rounded-xl p-5 shadow-sm space-y-4">
                            ...
                            <div className="space-y-3.5 text-xs font-semibold text-gray-600 dark:text-gray-300">
                                ...
                                <div className="space-y-1.5">
                                    <span className="text-gray-400 font-medium block">Địa chỉ giao nhận hàng</span>
                                    <div className="bg-neutral-50 dark:bg-bg-surface-850 p-3 rounded-lg border border-neutral-200 dark:border-stroke-sub-300 text-gray-800 dark:text-static-white text-xs leading-relaxed font-bold">
                                        {order.shipmentType === 'GTN' ? (
                                            <span>
                                                {order.addressDetail}, {order.commune}, {order.district}, {order.province}
                                            </span>
                                        ) : (
                                            <span>Nhận tại cửa hàng địa chỉ: {order.addressDetail || 'Cửa hàng CellphoneS chính thức'}</span>
                                        )}
                                    </div>
                                </div>
                        ...
                        {/* Support Info Card */}
                        {/* <div className="bg-white dark:bg-bg-weak-50 border border-neutral-200 dark:border-stroke-sub-300 rounded-xl p-4 shadow-sm flex items-center justify-between gap-4">
                            <div className="flex gap-2.5 items-center">
                                <div className="size-9 rounded-full bg-red-50 dark:bg-red-955/20 text-[#d70018] flex items-center justify-center shrink-0">
                                    <MapPin size={16} />
                                </div>
                                <div className="space-y-0.5 text-left">
                                    <span className="text-[10px] text-gray-400 font-bold block">Địa điểm nhận máy hỗ trợ</span>
                                    <span className="text-xs font-black text-gray-800 dark:text-white block line-clamp-1">
                                        {order.shipmentType === 'GTN' ? 'Trung tâm CSKH CellphoneS' : (order.addressDetail || 'Cửa hàng chính hãng')}
                                    </span>
                                </div>
                            </div>
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="flex items-center gap-1 text-[10px] border border-[#d70018] text-[#d70018] font-black px-2.5 py-1.5 rounded-lg hover:bg-red-50/5 transition-all shrink-0 cursor-pointer"
                            >
                                <Navigation size={12} />
                                Chỉ đường
                            </a>
                    ...
                    <div className="w-full md:col-span-5 space-y-4">
                        ...
                        <div className="bg-white dark:bg-bg-weak-50 border border-neutral-200 dark:border-stroke-sub-300 rounded-xl shadow-sm overflow-hidden">
                            ...
                            <div className="divide-y divide-neutral-100 dark:divide-stroke-sub-300">
                                ...
                                <div className="px-4 py-3.5 space-y-2.5 text-xs font-semibold text-gray-600 dark:text-gray-300">
                                    {calculatedDiscount !== undefined && calculatedDiscount > 0 && (
                                        <div className="flex justify-between">
                        ...
                        <div className="space-y-3">
                            {order.paymentType === 'QR' && (order.paymentStatus !== 'SUCCESS' && order.paymentStatus !== 'COD') && (
                                <Link
                                    to={`/users/payment/${order.id}` as any}
                                    className="w-full bg-[#d70018] hover:bg-red-700 active:scale-[0.98] text-white py-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm font-bold uppercase tracking-wider text-xs cursor-pointer text-center"
                                >
