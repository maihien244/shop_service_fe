...
export type BaseCategoryFilterParams = {
    'name:ct'?: string;
    'code:eq'?: string;
    isActive?: number;
};

export type ListBaseCategoryFilterProps = {
    filter: BaseCategoryFilterParams;
    onChangeFilter: React.Dispatch<React.SetStateAction<BaseCategoryFilterParams>>;
    onClearFilter: () => void;
};

export function ListBaseCategoryFilter({ filter, onChangeFilter, onClearFilter }: ListBaseCategoryFilterProps) {
        ...
        >
            <div className="absolute right-0 top-full mt-3 w-[380px] z-100 origin-top-left animate-in fade-in zoom-in-95 duration-200">
                <section className="shadow-complex border border-stroke-soft-200 rounded-12 bg-bg-white-0 dark:bg-bg-weak-50 dark:border-stroke-sub-300 w-full overflow-hidden">
                    ...
                    <div className="flex flex-col items-start gap-y-6 px-4 py-4 max-h-[70vh] overflow-y-auto">
